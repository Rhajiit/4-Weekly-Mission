...
const SubFolderBtn = styled.button<any>`
  display: flex;
  align-items: center;
  text-align: center;
  gap: 4px;
...
type EmptyLinkType = { isLoading: boolean };

const EmptyLink = function ({ isLoading }: EmptyLinkType) {
...
function SubFolders({ subFolderData, handleCurrentFolderChange }: any) {
  ...
  return (
    <SubFolderBtnList>
      <SubFolderBtn
        key={0}
        $state:any={(selectedBtn === 0).toString()}
...
function CurrentSubFolder({ currentFolder }: any) {
...
export default function LinkSubFolder() {
  ...
  const subFolderData = async (requestQuery: any) => {
